 #!/bin/bash
  ...
  # V.2.(2012)
	...
	then
		for t in $DBS ; do
			if  [ $t == "development" -o $t == "pspesslingen" -o $t == "wohlfahrtswerk" -o $t == "test1" -o $t == "test2" -o $t == "test3" -o $t == "test4" -o $t == "test5" -o $t == "test6" -o $t == "test7" -o $t == "test8" -o $t == "test9" -o $t == "test10" ]
			then
				#flag=0
				
				CheckData # Check if the field exits in the Columnseim table of the database, and it is a string type.
					
				if [ "${row[5]}" != "STRING" ] || [ "${row[3]}" == "" ] || [ "${row[4]}" == "" ] || [ "${row[5]}" == "" ] || [ "${row[4]}" != "$FIELD" ] || [ "${row[3]}" != "$TABLE0" ]
				then
					...
					D_T=`date +%Y%m%d`   
					mysql -u root -p$PS -D $t -e "select * FROM SYSCOLUMNSEIM WHERE TABLENAME LIKE '$TABLE0' AND COLNAME LIKE '$FIELD' AND DICTIONARYKEY LIKE '$TABLE.$FIELD';" ls -al 2>&1 | tee -a eraser.$D_T.log
					#mysql -u root -p$PS -D $t -e "DELETE FROM SYSCOLUMNSEIM WHERE TABLENAME LIKE '$TABLE0' AND COLNAME LIKE '$FIELD' AND DICTIONARYKEY LIKE '$TABLE.$FIELD';" 2>&1 | tee -a eraser.$D_T.log
					#mysql -u root -p$PS -D $t -e "ALTER TABLE $TABLE0 DROP $FIELD;" 2>&1 | tee -a eraser.$D_T.log
	...
	then
		...
		for t in $DBS ; do
			...
			then	
				...
				if [ "${row[5]}" != "SELECTIONVALUELIST" ] || [ "${row[3]}" == "" ] || [ "${row[4]}" == "" ] || [ "${row[5]}" == "" ] || [ "${row[4]}" != "$FIELD" ] || [ "${row[3]}" != "$TABLE0" ]
				then
					echo "** Database '$t'. The data introduced is not correct or not exists in this database."
					echo -e "\n"
			 		((cont++))
					...
					D_T=`date +%Y%m%d`
					mysql -u root -p$PS -D $t -e "DELETE FROM SELECTIONVALUESRELATION WHERE COLNAME LIKE '$FIELD' AND TYPE LIKE '$TABLE';" 1>&2 | tee -a eraser.$D_T.log
					mysql -u root -p$PS -D $t -e "DELETE FROM SELECTIONVALUES WHERE COLNAME LIKE '$FIELD' AND DICTIONARYKEY LIKE '$TABLE.$FIELD%' AND TYPE LIKE '$TABLE';" 2>&1 | tee -a eraser.$D_T.log
					mysql -u root -p$PS -D $t -e "DELETE FROM SYSCOLUMNSEIM WHERE TABLENAME LIKE '$TABLE0' AND COLNAME LIKE '$FIELD';" 2>&1 | tee -a eraser.$D_T.log
