 #!/bin/bash
  ...
  #								   *saveDB.Link with the showSelectDataBases function and control the repetion of the ddbb selected.  		  
	...
	then
		if [ $TYPE == "STRING" -o $TYPE == "INT" -o $TYPE == "BOOLEAN" -o $TYPE == "DATETIME" ]
		then
			deleteSimpleField
		else
	...
	while [ $c -gt 0 -o $c -eq 0 ]; do
		t=${selectedDB[c]}
		mysql -u root -p$PS -D $t -e "DELETE FROM SYSCOLUMNSEIM WHERE TABLENAME LIKE '$TABLE0' AND COLNAME LIKE '$FIELD' AND DICTIONARYKEY LIKE '$TABLE.$FIELD';" | tee -a $LOG 
	...
	while [ $c -gt 0 -o $c -eq 0 ]; do
		t=${selectedDB[c]}				
		mysql -u root -p$PS -D $t -e "DELETE FROM SELECTIONVALUESRELATION WHERE COLNAME LIKE '$FIELD' AND TYPE LIKE '$TABLE';"  | tee -a $LOG
		mysql -u root -p$PS -D $t -e "DELETE FROM SELECTIONVALUES WHERE COLNAME LIKE '$FIELD' AND DICTIONARYKEY LIKE '$TABLE.$FIELD%' AND TYPE LIKE '$TABLE';" | tee -a $LOG
		mysql -u root -p$PS -D $t -e "DELETE FROM SYSCOLUMNSEIM WHERE TABLENAME LIKE '$TABLE0' AND COLNAME LIKE '$FIELD';" | tee -a $LOG
		mysql -u root -p$PS -D $t -e "ALTER TABLE $TABLE0 DROP $FIELD;" | tee -a $LOG
	...
	then
		...
		# Check if the data exits
			if  [ "${row[5]}" == "" ] || [ "${row[3]}" == "" ] || [ "${row[4]}" == "" ] || [ "${row[5]}" == "" ] || [ "${row[4]}" != "$FIELD" ] || [ "${row[3]}" != "$TABLE0" ] # Don´t exists
			then
	...
	if [ -z "$DDBB" ];then #echo "Select at less one database"
		...
		do
			if [ $selectedNumber -lt 0 ] || [ $selectedNumber -gt $a ] || [[ $selectedNumber != [0-9]* ]] #! [[ $selectedNumber =~ ^[0-9]+$ ]]
